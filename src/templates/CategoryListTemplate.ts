...
export default class CategoryListTemplate implements DOMList, Observer<CategoryItem> {
  ...
  private constructor() {
    this.uls = document.querySelectorAll(".category__list") as NodeListOf<HTMLUListElement>;
  }
  ...
  render(categoryList: CategoryList): void {
    this.uls.forEach((ul) => {
      ...
      categoryList.categories.forEach((category) => {
        ...
        const categoryItem = `
          <span class="category__item" title="${name}" aria-label="${name}" data-category-id="${id}">

            <span class="category__item--count">${numberOfItems} task${plural} </span>

            <h4 class="category__item--title">${category.name}</h4>

            <div class="category__item--progressBar" style="--progress: ${completionPercentage}%;">
